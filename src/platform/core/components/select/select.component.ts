...
export class NovoSelectComponent
  ...
  @ContentChildren(NovoOptionComponent, { descendants: true })
  public _options: QueryList<NovoOptionComponent>;
  @ViewChildren(NovoOptionComponent)
  public _others: QueryList<NovoOptionComponent>;
  ...
  public get options(): any[] {
    let content: NovoOptionComponent[] =
      this._options && this._options.length ? this._options.toArray() : [];
    let view: NovoOptionComponent[] =
      this._others && this._others.length ? this._others.toArray() : [];
  ...
  public setupSelectionSubscription(): void {
    this.destroySelectionSubscription();
    this._selectionSubscription = this.optionSelections.subscribe(
      (selection: NovoSelectionEvent) => {
  ...
  get optionSelections(): Observable<any> {
    return merge(
      ...this.options.map((option: NovoOptionComponent) => option.selection),
    );
  }
  ...
  public setValue(option: NovoOptionComponent): void {
    if (option) {
      let index: number = this.options.findIndex(
  ...
  public clear(): void {
    this.setValue(
      this.options.find((i: NovoOptionComponent) => i.value === undefined),
    );
    this.header = {
      open: false,
      valid: true,
  ...
  public onKeyDown(event: KeyboardEvent): void {
    if (this.panelOpen) {
      ...
      if (event.keyCode === UP_ARROW && this.selectedIndex > 0) {
        ...
        let item: any = this.options.find(
          (i: any) => i.viewValue.toUpperCase().indexOf(this.filterTerm) === 0,
        );
        if (item) {
  ...
  public scrollToIndex(index: number): void {
    ...
    let item: HTMLElement = items[
      this.headerConfig ? index + 1 : index
    ] as HTMLElement;
    if (item) {
  ...
  public writeValue(model: any): void {
    ...
    if (this.options && this.options.length) {
      let item: NovoOptionComponent = this.options.find(
        (i: any) => i.value === model,
      );
      if (item) {
        this.setValue(item);
