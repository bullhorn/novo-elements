...
export function getNovoAutocompleteMissingPanelError(): Error {
  return Error(
    'Attempting to open an undefined instance of `novo-autocomplete`. ' +
...
export class NovoAutocompleteDirective
  ...
  get panelClosingActions(): Observable<any> {
    return merge(
      ...
      this.autocomplete._keyManager.tabOut.pipe(
        filter(() => this.autocomplete.panelOpen),
      ),
      this._escapeEventStream,
      this._outsideClickStream,
      this.autocomplete
        ? this.autocomplete.overlay._overlayRef
            .detachments()
            .pipe(filter(() => this.autocomplete.panelOpen))
        : observableOf(),
    );
  }
  ...
  get optionSelections(): Observable<any> {
    return merge(
      ...this.autocomplete.options.map(
        (option: NovoListItemComponent) => option.select,
      ),
    );
  }
  ...
  private _scrollToOption(): void {
    ...
    const activeOptionIndex: number =
      this.autocomplete._keyManager.activeItemIndex || 0;
    const labelCount: number = 0;
    const optionOffset: number =
      (activeOptionIndex + labelCount) * AUTOCOMPLETE_OPTION_HEIGHT;
  ...
  private _subscribeToClosingActions(): Subscription {
    const firstStable: Observable<
      any
    > = this._zone.onStable.asObservable().pipe(take(1));
    const optionChanges: Observable<
      any
    > = this.autocomplete.options.changes.pipe(delay(0));
