...
export class NovoAutocompleteComponent implements AfterContentInit {
  ...
  /** @docs-private */
  @ContentChildren(NovoListItemComponent, { descendants: true })
  public options: QueryList<NovoListItemComponent>;
  ...
  set classList(classList: string) {
    if (classList && classList.length) {
      classList
        .split(' ')
        .forEach(
          (className: string) => (this._classList[className.trim()] = true),
        );
  ...
  public ngAfterContentInit(): void {
    this._keyManager = new ActiveDescendantKeyManager<NovoListItemComponent>(
      this.options,
    ).withWrap();
  ...
  public _emitSelectEvent(option: NovoListItemComponent): void {
    const event: NovoAutocompleteSelectedEvent = new NovoAutocompleteSelectedEvent(
      this,
      option,
    );
