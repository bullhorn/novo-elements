...
describe('Pipe: GroupByPipe', () => {
  ...
  it('should not change anything if not array', () => {
    ...
    expect(pipe.transform(null)).toEqual(null); // tslint:disable-line
  ...
  it('group on multiple discriminator', () => {
    ...
    const result: any = pipe.transform(arrayWithDiscriminator, ['key', 'type']);
    expect(result).toEqual({
  ...
  it('group on deep property discriminator', () => {
    ...
    const result: any = pipe.transform(arrayWithDiscriminator, 'prop.deep');
    expect(result).toEqual({
