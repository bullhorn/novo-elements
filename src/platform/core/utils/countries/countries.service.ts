...
export const COUNTRIES: ICountryDef[] = [
    {
        id: 1,
        code: 'US',
        name: 'United States',
 ...
 * Get country by id
 */
export function findByCountryId(id: number): ICountryDef {
    return COUNTRIES.find((c: ICountryDef) => c.id === id);
}

/**
 * Find country object by name
 */
export function findByCountryName(name: string): ICountryDef {
    return COUNTRIES.find((c: ICountryDef) => c.name === name.trim());
}

/**
 * Find country object by code
 */
export function findByCountryCode(code: string): ICountryDef {
    return COUNTRIES.find((c: ICountryDef) => c.code === code.trim());
}
...
export function getStates(name: string): string[] {
    let country: ICountryDef = COUNTRIES.find((c: ICountryDef) => c.name === name.trim());
    return country ? country.states.map((s: IStateDef) => s.name) : [];
}

/**
 * Get states by country name
 */
export function getStateObjects(name: string): IStateDef[] {
    let country: ICountryDef = COUNTRIES.find((c: ICountryDef) => c.name === name.trim());
