// NG2
import { Component, Input, OnInit, HostBinding, OnChanges, SimpleChanges, Output, EventEmitter } from '@angular/core';
// VENDOR
...
export class NovoValueElement implements OnInit, OnChanges {
  ...
  @Input() retrieveText: Subject<any> = new Subject();
  @Output() onRetrieveText: EventEmitter<string> = new EventEmitter<string>();
  ...
  constructor(public render: RenderPipe) {}
  ...
  ngOnInit() {
    ...
    this.retrieveText.subscribe((event) => {
      this.retrieveTextHandler();
    });
  }
  ...
  retrieveTextHandler(): void {
    let text: string = '';
    let renderValue: any = this.render.render(this.data, this.meta);
    if (renderValue && Helpers.isString(renderValue.changingThisBreaksApplicationSecurity)) {
      text = renderValue.changingThisBreaksApplicationSecurity;
    } else {
      text = renderValue;
    }
