...
import { dropdownSelectors } from '../utils/DropdownUtil';
...
describe('Dropdown Demo Page', () => {
    ...
    describe('Page Elements', () => {
        it('should display page title and examples', async () => {
            ...
            await verifyPresent('dropdown-examples-page', 'dropdown examples page');
        });
    ...
    describe('Dropdown Menu', () => {
        ...
        it('should open a dropdown with options and opt-group label', async () => {
            ...
            await verifyPresent(dropdownSelectors.container, 'dropdown container');
            await verifyPresent(automationId('opt-group-engage-label'), 'opt-group engage label');
            await verifyText(`${automationId('opt-group-engage-label')} ${dropdownSelectors.optGroupLabel}`, 'Engage');
        });

        it('should close the dropdown when clicked again', async () => {
            await click(automationId('basic-dropdown-actions-button'));
            await verifyAbsent(dropdownSelectors.container, 'dropdown container');
        });
