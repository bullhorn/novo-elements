...
describe('ng-add schematic', () => {
  ...
  it('should update package.json', async () => {
    ...
    expect(dependencies['@angular/animations'])
      .withContext('Expected the @angular/animations package to have the same ' + 'version as @angular/core.')
      .toBe(angularCoreVersion);
  ...
  it('should add default theme', async () => {
    ...
    expectProjectStyleFile(project, './node_modules/@angular/material/prebuilt-themes/indigo-pink.css');
  });
  ...
  it('should support adding a custom theme', async () => {
    ...
    const tree = await runner.runSchematicAsync('ng-add-setup-project', { theme: 'custom' }, appTree).toPromise();
  ...
  it('should create a custom theme file if no SCSS file could be found', async () => {
    ...
    expect(tree.files).withContext('Expected a custom theme file to be created').toContain(expectedStylesPath);
  ...
  it('should add font links', async () => {
    ...
    indexFiles.forEach((indexPath) => {
      ...
      expect(htmlContent).toContain('  <link href="https://fonts.googleapis.com/icon?family=Material+Icons"');
      expect(htmlContent).toContain('  <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@' + '300;400;500&display=swap"');
    });
  });
  ...
  it('should add material app styles', async () => {
    ...
    expect(htmlContent).toContain('body { margin: 0; font-family: Roboto, "Helvetica Neue", sans-serif; }');
  });
  ...
  describe('animations disabled', () => {
    ...
    it('should not add NoopAnimationsModule if BrowserAnimationsModule is set up', async () => {
      ...
      // to noop animations.
      const fileContent = addModuleImportToRootModule(appTree, 'BrowserAnimationsModule', '@angular/platform-browser/animations', project);
  ...
  describe('custom project builders', () => {
    ...
    it('should throw an error if the "build" target has been changed', async () => {
      ...
      await expectAsync(runner.runSchematicAsync('ng-add-setup-project', {}, appTree).toPromise()).toBeRejectedWithError(
        /not using the default builders.*build/,
      );
    });
    ...
    it('should warn if the "test" target has been changed', async () => {
      ...
      expect(warnOutput[0]).toMatch(/not using the default builders.*cannot add the configured theme/);
    });
  });

  describe('theme files', () => {
    /** Path to the default prebuilt theme file that will be added when running ng-add. */
    const defaultPrebuiltThemePath = './node_modules/@angular/material/prebuilt-themes/indigo-pink.css';
    ...
    it('should replace existing prebuilt theme files', async () => {
      ...
      expect(styles).not.withContext('Expected the existing prebuilt theme file to be removed.').toContain(existingThemePath);
      expect(styles).withContext('Expected the default prebuilt theme to be added.').toContain(defaultPrebuiltThemePath);
    });
    ...
    it('should not replace existing custom theme files', async () => {
      ...
      expect(styles).not.toContain(defaultPrebuiltThemePath, 'Expected the default prebuilt theme to be not configured.');
      expect(errorOutput.length).toBe(1);
    ...
    it('should not add a theme file multiple times', async () => {
      ...
      expect(styles)
        .withContext('Expected the "styles.css" file and default prebuilt theme to be ' + 'the only styles')
