...
describe('v9 HammerJS removal', () => {
  const GESTURE_CONFIG_TEMPLATE_PATH = resolveBazelPath(__dirname, '../../../migrations/hammer-gestures-v9/gesture-config.template');
  ...
  it('should not throw if project tsconfig does not have explicit root file names', async () => {
    ...
    await runner.runExternalSchematicAsync('@schematics/angular', 'application', { name: 'second-project' }, tree).toPromise();
  ...
  describe('hammerjs not used', () => {
    ...
    it('should report error if unable to remove reference to gesture config', async () => {
      ...
      expect(logOutput).toContain(
        `projects/cdk-testing/src/test.module.ts@9:42 - ` + `Cannot remove reference to "GestureConfig". Please remove manually.`,
      );
      expect(logOutput).toContain(
        `projects/cdk-testing/src/test.module.ts@10:56 - ` + `Cannot remove reference to "GestureConfig". Please remove manually.`,
      );
      expect(logOutput).toContain(
        `projects/cdk-testing/src/test.module.ts@16:21 - ` + `Cannot remove reference to "GestureConfig". Please remove manually.`,
      );
      expect(tree.readContent('/projects/cdk-testing/src/test.module.ts')).toContain(dedent`
        import {NgModule} from '@angular/core';
        import {NOT_KNOWN_TOKEN, HAMMER_GESTURE_CONFIG} from '@angular/platform-browser';
  ...
  describe('hammerjs used programmatically', () => {
    ...
    it('should ignore global reference to Hammer if not resolved to known types', async () => {
      ...
      expect(tree.readContent('/projects/cdk-testing/src/main.ts')).not.toContain(`import 'hammerjs';`);
    });
  ...
  describe('used in template with custom Material gesture events', () => {
    ...
    it('should create gesture config file', async () => {
      ...
      expect(tree.readContent('/projects/cdk-testing/src/gesture-config.ts')).toBe(readFileSync(GESTURE_CONFIG_TEMPLATE_PATH, 'utf8'));
    });
    ...
    it('should create gesture config file with different name if it would conflict', async () => {
      ...
      expect(tree.readContent('/projects/cdk-testing/src/gesture-config-1.ts')).toBe(readFileSync(GESTURE_CONFIG_TEMPLATE_PATH, 'utf8'));
    });
    ...
    it('should add gesture config provider to app module if module is referenced through ' + 're-exports in bootstrap', async () => {
      writeFile(
        '/projects/cdk-testing/src/app/app.component.html',
        `
        <span (slide)="onSlide($event)"></span>
  ...
  it('should not remove hammerjs from "package.json" file if used in one project while ' + 'unused in other project', async () => {
    addPackageToPackageJson(tree, 'hammerjs', '0.0.0');
  ...
  describe('with custom gesture config', () => {
    ...
    it('should warn if hammer is used in template and references to Material gesture config ' + 'were detected', async () => {
      writeFile(
        '/projects/cdk-testing/src/test.component.ts',
        dedent`
          import {HAMMER_GESTURE_CONFIG} from '@angular/platform-browser';
      ...
      expect(tree.readContent('/projects/cdk-testing/src/test.component.ts')).toContain(dedent`
          ...
          export class TestModule {}`);
      expect(tree.readContent('/projects/cdk-testing/src/sub.module.ts')).toBe(subModuleFileContent);
    });
