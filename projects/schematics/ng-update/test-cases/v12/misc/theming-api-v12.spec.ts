...
describe('v12 theming API migration', () => {
  ...
  it('should detect imports using double quotes', async () => {
    ...
    expect(splitFile(THEME_PATH)).toEqual([`@use '@angular/material' as mat;`, `@include mat.core();`]);
  });

  it('should migrate mixins that are invoked without parentheses', async () => {
    writeLines(THEME_PATH, [`@import '@angular/material/theming';`, `@include mat-base-typography;`]);

    await runMigration();

    expect(splitFile(THEME_PATH)).toEqual([`@use '@angular/material' as mat;`, `@include mat.typography-hierarchy;`]);
  });

  it('should migrate files that import the Material APIs transitively', async () => {
    writeLines(THEME_PATH, [`@import 're-exports-material-symbols';`, `@include mat-core();`, `@include mat-button-theme();`]);
  ...
  it('should insert the new @use statement above other @import statements', async () => {
    writeLines(THEME_PATH, [`@import './foo'`, `@import "@angular/material/theming";`, `@import './bar'`, `@include mat-core();`]);
  ...
  it('should account for other @use statements when inserting the new Material @use', async () => {
    writeLines(THEME_PATH, [`@use './foo'`, `@import './bar'`, `@import "@angular/material/theming";`, `@include mat-core();`]);

    await runMigration();

    expect(splitFile(THEME_PATH)).toEqual([`@use '@angular/material' as mat;`, `@use './foo'`, `@import './bar'`, `@include mat.core();`]);
  });
  ...
  it('should account for single-line comment file headers placed aboved the @import statements', async () => {
    writeLines(THEME_PATH, [`// This is a license.`, `@import './foo'`, `@import '@angular/material/theming';`, `@include mat-core();`]);
  ...
  it('should migrate multiple files within the same project', async () => {
    ...
    writeLines(THEME_PATH, [`@import '@angular/material/theming';`, `@include angular-material-theme();`]);
    ...
    expect(splitFile(THEME_PATH)).toEqual([`@use '@angular/material' as mat;`, `@include mat.all-component-themes();`]);
  ...
  it('should drop the old import path even if the file is not using any symbols', async () => {
    writeLines(THEME_PATH, [`@import '@angular/material/theming';`, ``, `.my-dialog {`, `color: red;`, `}`]);
  ...
  it('should not migrate files in the node_modules', async () => {
    writeLines('/node_modules/theme.scss', [`@import '@angular/material/theming';`, ``, `@include mat-button-toggle-theme();`, ``]);
  ...
  it('should only migrate unprefixed variables if there is a theming import', async () => {
    ...
    writeLines(THEME_PATH, [`@import '@angular/material/theming';`, ``, `.my-button {`, `z-index: $z-index-fab;`, `}`]);

    writeLines(otherTheme, [`@import 're-exports-material-symbols';`, ``, `.my-drawer {`, `z-index: $z-index-drawer;`, `}`]);

    await runMigration();

    expect(splitFile(THEME_PATH)).toEqual([`.my-button {`, `z-index: 20;`, `}`]);

    expect(splitFile(otherTheme)).toEqual([`@import 're-exports-material-symbols';`, ``, `.my-drawer {`, `z-index: $z-index-drawer;`, `}`]);
  });
  ...
  it('should not rename variables appended with extra characters', async () => {
    writeLines(THEME_PATH, [`@import '@angular/material/theming';`, `$mat-light-theme-background-override: 123;`, `@include mat-core();`]);
  ...
  it('should not migrate commented out code', async () => {
    ...
    expect(splitFile(THEME_PATH)).toEqual([`// @import '@angular/material/theming';`, '/* @include mat-core(); */']);
  });

  it('should not migrate single-line commented code at the end of the file', async () => {
    writeLines(THEME_PATH, [`// @import '@angular/material/theming';`, '// @include mat-core();', '// @include mat-button-theme();']);
