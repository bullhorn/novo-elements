...
export class MiscTemplateMigration extends Migration<null> {
  ...
  visitTemplate(template: ResolvedResource): void {
    ...
    findOutputsOnElementWithTag(template.content, 'selectionChange', ['mat-list-option']).forEach((offset) => {
      this.failures.push({
    ...
    findOutputsOnElementWithTag(template.content, 'selectedChanged', ['mat-datepicker']).forEach((offset) => {
      this.failures.push({
        filePath: template.filePath,
        position: template.getCharacterAndLineOfPosition(template.start + offset),
        message:
    ...
    findInputsOnElementWithTag(template.content, 'selected', ['mat-button-toggle-group']).forEach((offset) => {
      this.failures.push({
        filePath: template.filePath,
        position: template.getCharacterAndLineOfPosition(template.start + offset),
