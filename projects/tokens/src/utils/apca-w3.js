...
function reverseAPCA(
  ...
  if (Math.abs(contrast) < 5) {
    return false;
  } // abs contrast must be > 5
  ...
  const mFactor = 1.9468554433171;
  const mFactInv = 1 / mFactor;
  const mOffsetIn = 0.0387393816571401;
  const mExpAdj = 0.283343396420869;
  const mExp = mExpAdj / blkClmp;
  const mOffsetOut = 0.312865795870758;

  contrast = (parseFloat(contrast) * 0.01 + offset) / scale;

  // Soft clamps Y if it is near black.
  knownY =
    knownY > blkThrs ? knownY : knownY + Math.pow(blkThrs - knownY, blkClmp);
  ...
  unknownY =
    unknownY > blkThrs
      ? unknownY
      : Math.pow((unknownY + mOffsetIn) * mFactor, mExp) * mFactInv -
        mOffsetOut;
...
function fontLookupAPCA(contrast) {
  ...
  for (; w < weightArrayLen; w++) {
    ...
    if (tempFont > 400) {
      ...
      tempFont > 24
        ? (returnArray[w] = Math.round(
            tempFont - fontDeltaAscend[index][w] * scoreAdj
          ))
        : (returnArray[w] =
            tempFont -
            ((2.0 * fontDeltaAscend[index][w] * scoreAdj) | 0) * 0.5);
      // (n|0) is bw floor
    }
  }
  /////////\  End Interpolation   ////////////////////////////////////////////
