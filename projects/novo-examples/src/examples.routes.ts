...
@Component({
  ...
  template: `<h1>Avatar <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/avatar">(source)</a></h1><p>Avatars are images used to represent users and organizations on GitHub. They typically are squares with rounded edges.</p><h2>Types</h2><h5>Avatar</h5><p>Avatars are images used to represent users and organizations on GitHub. They typically are squares with rounded edges.</p><p><code-example example="avatar-usage"></code-example></p><h5>Avatar Stack</h5><p>AvatarStack is used to display more than one Avatar in an inline stack.</p><p><code-example example="avatar-stack-usage"></code-example></p>`
})
...
@Component({
  ...
  template: `<h1>Breadcrumb <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/breadcrumb">(source)</a></h1><p>A breadcrumb displays the current location within a hierarchy. It allows going back to states higher up in the hierarchy.</p><h2>When To Use</h2><ul><li>When the system has more than two layers in a hierarchy.</li><li>When you need to inform the user of where they are.</li><li>When the user may need to navigate back to a higher level.</li><li>When the application has multi-layer architecture.</li></ul><h2>Examples</h2><h5>Static</h5><p>Breadcrumbs can be use statical with the <code>novo-breadcrumb</code> and <code>novo-breadcrumb-item</code> elements. You can also set the breadcrumb item to have a menu too.</p><p><code-example example="breadcrumb-usage"></code-example></p><h5>Dynamic</h5><p>If you need to build the breadcrumbs dynamically based on data within the app you can use the <code>source</code> attribute pass the values of the breadcrumb.</p><p><code-example example="breadcrumb-source-usage"></code-example></p>`
})
...
@Component({
  selector: 'field-page',
  template: `<h1>Field <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/field">(source)</a></h1><p>A Field is a component used to wrap several Angular Material components and apply common Text field styles such as the underline, label and hint messages.</p><p>In this document, &quot;form field&quot; refers to the wrapper component <code>&lt;novo-form-field&gt;</code> and &quot;form field control&quot; refers to the component that the <code>&lt;novo-form-field&gt;</code> is wrapping (e.g. the input, textarea, select, etc.)</p><h5>Works with following input types</h5><ul><li>Default input, select, textarea</li><li>novo-select</li><li>novo-datepicker</li></ul><h2>Examples</h2><h5>Basic Usage</h5><p>tbw...</p><p><code-example example="field-usage"></code-example></p><h5>Anatomy</h5><p>Form fields consists of the following parts:</p><ul><li>Container</li><li>Leading icon (Optional element)</li><li>Label</li><li>Placeholder/Input text</li><li>Trailing icon (Optional element)</li><li>Helper text/Error text (Optional element)</li></ul><p>See how they are used below...</p><p><code-example example="field-anatomy"></code-example></p><h5>Native Controls</h5><p>The following input types can be used with novoInput:</p><ul><li>Text: text, password, email, search, tel, url, number</li><li>Date: date, datetime-local, month, week, time</li><li>Other: color, range</li></ul><p><code-example example="field-native"></code-example></p>`
})
...
@Component({
  selector: 'menu-page',
  template: `<h1>Menu <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/dropdown">(source)</a></h1><p>Menu allow users to take an action by selecting from a list of choices revealed upon opening a temporary menu.</p><h2>Types</h2><h5>Basic Menu</h5><p>This is a simple menu.</p><p><code-example example="basic-menu"></code-example></p><h5>Nested Menu</h5><p>This is an example of how you can have sub-menus for each menu item.</p><p><code-example example="nested-menu"></code-example></p><h5>Menu Context</h5><p>TBW of an example when you can define a context to pass to the context menu to control visibility or to use on the callback action.</p><p><code-example example="menu-context"></code-example></p>`
})
...
@Component({
  ...
  template: `<h1>Non Ideal State <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/non-ideal-state">(source)</a></h1><p>Non-ideal UI states inform the user that some content is unavailable. There are several types of non-ideal states, including:</p><ul><li><strong>Empty state</strong>: when a list has no data in it yet, or a container's contents have been intentionally removed.</li><li><strong>Loading state</strong>: when waiting for data to load, Best practice is to show a spinner for this state, with optional explanatory text below the spinner.</li><li><strong>Error state</strong>: its broken (for instance, 404 and 500 HTTP errors). In this case, best practice is to add a call to action directing the user what to do next.</li></ul><h2>Examples</h2><h5>Basic Usage</h5><p>Basic use-case is to display an icon, message, and reason for this state to occur. And provide a call to action for the user.</p><p><code-example example="non-ideal-state-usage"></code-example></p><p>The call to action doesn't necessarily need to be a button, for example:</p><p><code-example example="non-ideal-state-search-usage"></code-example></p>`
})
...
@Component({
  ...
  template: `<h1>Progress <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/avatar">(source)</a></h1><p>Progress bars are used to show how much of a task is complete, like loading data.</p><h2>Types</h2><h5>Progress Bar</h5><p>Progress bars are generally linear and can show the percent complete of a task. Progress can also be indeterminate.</p><p><code-example example="progress-bar-usage"></code-example></p><h5>Radial Progress Bars</h5><p>Radial Progress can be used to show a percent complete or multiple statuses in a circle.</p><p><code-example example="progress-bar-radial-usage"></code-example></p>`
})
...
@Component({
  ...
  template: `<h1>Color Pickers</h1><p>These allow users to easily select a color swatch. It comes in a handful of varieties based on the content of the field.</p><h2>Color Picker <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/date-picker">(source)</a></h2><p>The color picker is used to select a color. It appears in all date picker fields.</p><h5>Basic Usage</h5><p><code-example example="color-picker"></code-example></p><h5>Color Input Example</h5><p><code-example example="color-input"></code-example></p>`
})
...
@Component({
  selector: 'aside-page',
  template: `<h1>Asides <a href="https://github.com/bullhorn/novo-elements/blob/master/projects/novo-elements/src/elements/aside">(source)</a></h1><p>Asides are pop-up dialogues designed to grab attention and inform the user of something critical, force a decision, or extend a workflow. There are two categories of modals: notification and workflow. Regardless of type, a modal should have a maximum of two main buttons.</p><h2>Examples</h2><h5>Custom</h5><p>In the case where &quot;Success&quot;, &quot;Warning&quot;, and &quot;Error&quot; notfications aren't enough, use the custom notification. Custom notifcations allow any of the Bullhorn Icons to be used in the notification.</p><p><code-example example="aside-usage"></code-example></p><h5>Add</h5><p>Add modals have a colored title bar based on the record type being created. Additionally, due to a greater than average amount of content, they have fixed footers.</p><p><code-example example="aside-form"></code-example></p>`
})
...
const routes: Routes = [
  ...
  { path: 'utils/aside', component: AsidePage, data: { title: 'Aside', section: 'utils' } },
...
export const PAGE_LIST = [
  AceEditorPage,AvatarPage,BreadcrumbPage,ButtonsPage,CalendarPage,ComponentsPage,DataTablePage,DropdownPage,FieldPage,IconPage,LoadingPage,MenuPage,NonIdealStatePage,ProgressPage,QuickNotePage,SearchPage,SlidesPage,SwitchPage,TabbedGroupPickerPage,TablePage,ColorsPage,CompositionPage,DesignPage,IconographyPage,TypographyPage,ChipsPage,ColorPickerPage,DatePickerPage,EditorPage,FormControlsPage,FormGroupsPage,FormPage,MultiPickerPage,PickerPage,RadioButtonsPage,SelectPage,TilesPage,ValuePage,HomePage,CardsPage,ExpansionPage,HeaderPage,LayoutsPage,ListPage,StepperPage,TabsPage,PatternsPage,TemplatesPage,AsidePage,ChomskyPage,FieldInteractionsPage,ModalPage,PipesPage,PopOverPage,SecurityPage,TipWellPage,ToasterPage,TooltipPage
];
