...
import { TestBed } from '@angular/core/testing'; // the object coming back from this function references itself.
...
class MockPostrobot {
  ...
  protected mockSendToMe(msg: MessageType, data: any, from: MockPostrobot): Promise<any> {
    ...
    return Promise.resolve(responder?.(mockRobotEvent)).then((replyData) => ({ data: { ...replyData } }));
  }
}
...
describe('AppBridge', () => {
  ...
  let handleFn: Mock<(handler: AppBridgeHandler, data: any) => any>;
  ...
  beforeEach(() => {
    ...
    hostBridge['windowOrigin'] =
      frame1Bridge['windowOrigin'] =
      frame2Bridge['windowOrigin'] =
        function () {
          return this.postRobot.mockOrigin;
        };
  ...
  it('should register a new app', async () => {
    ...
    expect(handleFn).toHaveBeenCalledWith(AppBridgeHandler.REGISTER, expect.objectContaining({ id: 'childapp1' }));
  ...
  it('opens a list', async () => {
    ...
    expect(handleFn).toHaveBeenCalledWith(AppBridgeHandler.OPEN_LIST, expect.objectContaining({ criteria: 'small' }));
  });

  // This would seem to be the correct, expected outcome of openList but it isn't happening right now.
  it.skip('opens a list - using type: List', () => {
    handleFn.mockReturnValue(true);
    frame1Bridge.openList({
      criteria: 'small',
      type: 'Candidate',
    ...
    expect(handleFn).toHaveBeenCalledWith(
      AppBridgeHandler.OPEN_LIST,
      expect.objectContaining({ criteria: 'small', type: 'List', entityType: 'Candidate' }),
    );
  });
  ...
  describe('Registration-dependent actions', () => {
    ...
    it('refreshes a window', async () => {
      ...
      expect(handleFn).toHaveBeenCalledWith(AppBridgeHandler.REFRESH, expect.anything());
    });

    it('pins a window', async () => {
      handleFn.mockReturnValue(true);
      await frame1Bridge.pin();
      expect(handleFn).toHaveBeenCalledWith(AppBridgeHandler.PIN, expect.objectContaining({ id: frame1Bridge.id, windowName: 'terry' }));
    });

    it('closes a window', async () => {
      handleFn.mockReturnValue(true);
      await frame1Bridge.close();
      expect(handleFn).toHaveBeenCalledWith(AppBridgeHandler.CLOSE, expect.objectContaining({ id: frame1Bridge.id, windowName: 'terry' }));
    });
    ...
    it('updates app title', async () => {
      ...
      expect(handleFn).toHaveBeenCalledWith(
        AppBridgeHandler.UPDATE,
        expect.objectContaining({ title: 'x', id: frame1Bridge.id, windowName: 'terry' }),
      );
    });
  });
      ...
      for (const cmd of Object.keys(cmdFunctionsWithArgs)) {
        it(`should return false when ${cmd}() receives ${failTerm}`, async () => {
          ...
          try {
            ...
            throw new Error(`Function passed unexpectedly when receiving ${failTerm}. Response: ${response}`);
          } catch (result) {
  ...
  it('should reject with null when postrobot transmission to parent fails', async () => {
    ...
    vi.spyOn(frame2Robot, 'sendToParent').mockReturnValue(Promise.reject(error));
    try {
  ...
  it('handles REQUEST_DATA failure', async () => {
    ...
    try {
      ...
      throw new Error('Should not request data successfully');
    } catch (err2) {
  ...
  it.skip('should fire custom events to parents of parents', async () => {
    let receivedEvent = false;
