...
@Component({
  ...
  template: `
    <!--Left Icon-->
    @if (((icon && side === 'left') || (secondIcon && secondSide() === 'left')) && !loading) {
      <i [ngClass]="leftSideIconClass()" class="novo-button-icon novo-button-icon-left"></i>
    }
    <!--Transcluded Content-->
    <span #textContent class="button-contents"><ng-content></ng-content></span>
    <!--Right Icon-->
    @if (((icon && side === 'right') || (secondIcon && secondSide() === 'right')) && !loading) {
      <i [ngClass]="rightSideIconClass()" class="novo-button-icon novo-button-icon-right"></i>
    }
    <!--Loading-->
    @if (loading) {
      <i class="loading novo-button-loading">
        <svg
          version="1.1"
