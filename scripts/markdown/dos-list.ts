...
function makeNode(token, TokenConstructor) {
  ...
  if (token.content.indexOf('(✓) ') === 0) {
    node.content = `<novo-icon color="success" mr="md" size="md">check</novo-icon><novo-text color="success">${content}</novo-text>`;
  } else if (token.content.indexOf('(x) ') === 0 || token.content.indexOf('(X) ') === 0) {
    node.content = `<novo-icon color="error" mr="md" size="md">times</novo-icon><novo-text color="error">${content}</novo-text>`;
  }
  return node;
}
